...
class NetworkLookup {
  ...
  find(options) {
    ...
    if (chainFamily && chainSelector !== undefined) {
      ...
      let network300 = getBySelector(this.maps.testnetBySelectorByFamily[chainFamily]);
      if (!network300) {
        network300 = getBySelector(this.maps.mainnetBySelectorByFamily[chainFamily]);
      }
      return network300;
    }
    if (chainFamily && chainSelectorName) {
      ...
      let network300 = this.maps.testnetByNameByFamily[chainFamily].get(chainSelectorName);
      if (!network300) {
        network300 = this.maps.mainnetByNameByFamily[chainFamily].get(chainSelectorName);
      }
      return network300;
    }
    if (chainSelector !== undefined) {
      ...
      let network300 = getBySelector(this.maps.testnetBySelector);
      if (!network300) {
        network300 = getBySelector(this.maps.mainnetBySelector);
      }
      return network300;
    }
    if (chainSelectorName) {
      ...
      let network300 = this.maps.testnetByName.get(chainSelectorName);
      if (!network300) {
        network300 = this.maps.mainnetByName.get(chainSelectorName);
      }
      return network300;
    }
    return;
...
var rpcCall = (runtime2, httpClient, method, params, id) => {
  ...
  return httpClient.sendRequest(runtime2, (sendRequester) => {
    ...
    if (!ok(r))
      throw new Error(`${method} HTTP error`);
    const resp = json(r);
    if (resp.error)
...
var onCronTrigger = (runtime2) => {
  const { tokenAddress, lookbackBlocks } = runtime2.config;
  const httpClient = new ClientCapability;
  const blockHex = JSON.parse(rpcCall(runtime2, httpClient, "eth_blockNumber", [], 1));
  const currentBlock = parseInt(blockHex, 16);
  const fromBlock = "0x" + Math.max(0, currentBlock - lookbackBlocks).toString(16);
  runtime2.log(`Polling Transfer events — blocks ${fromBlock} → latest (current: ${currentBlock})`);
  const logsRaw = rpcCall(runtime2, httpClient, "eth_getLogs", [{
    fromBlock,
    toBlock: "latest",
  ...
  if (logs.length === 0) {
    runtime2.log("No Transfer events in this window");
    return JSON.stringify({ events: 0, fromBlock, currentBlock });
  }
  runtime2.log(`Found ${logs.length} Transfer event(s)`);
  const events = logs.map((log) => {
    const from2 = "0x" + log.topics[1].slice(26);
    const to = "0x" + log.topics[2].slice(26);
    const amount = BigInt(log.data);
    runtime2.log(`Transfer: ${from2} → ${to}  amount=${amount.toString()} (raw)`);
    return { from: from2, to, amount: amount.toString(), txHash: log.transactionHash, block: parseInt(log.blockNumber, 16) };
  });
  return JSON.stringify({ events: events.length, fromBlock, currentBlock, transfers: events });
};
