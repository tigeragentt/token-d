...
(function(EncodingType2) {
  ...
  EncodingType2[EncodingType2["BASE58"] = 1] = "BASE58";
  EncodingType2[EncodingType2["BASE64"] = 2] = "BASE64";
  EncodingType2[EncodingType2["BASE64_ZSTD"] = 3] = "BASE64_ZSTD";
  EncodingType2[EncodingType2["JSON_PARSED"] = 4] = "JSON_PARSED";
  EncodingType2[EncodingType2["JSON"] = 5] = "JSON";
})(EncodingType || (EncodingType = {}));
var CommitmentType;
(function(CommitmentType2) {
  CommitmentType2[CommitmentType2["NONE"] = 0] = "NONE";
  CommitmentType2[CommitmentType2["FINALIZED"] = 1] = "FINALIZED";
  CommitmentType2[CommitmentType2["CONFIRMED"] = 2] = "CONFIRMED";
  CommitmentType2[CommitmentType2["PROCESSED"] = 3] = "PROCESSED";
})(CommitmentType || (CommitmentType = {}));
var ConfirmationStatusType;
(function(ConfirmationStatusType2) {
  ConfirmationStatusType2[ConfirmationStatusType2["NONE"] = 0] = "NONE";
  ConfirmationStatusType2[ConfirmationStatusType2["PROCESSED"] = 1] = "PROCESSED";
  ConfirmationStatusType2[ConfirmationStatusType2["CONFIRMED"] = 2] = "CONFIRMED";
  ConfirmationStatusType2[ConfirmationStatusType2["FINALIZED"] = 3] = "FINALIZED";
...
(function(ComparisonOperator2) {
  ...
  ComparisonOperator2[ComparisonOperator2["LT"] = 3] = "LT";
  ComparisonOperator2[ComparisonOperator2["GTE"] = 4] = "GTE";
...
init_secp256k1();
init_secp256k1();
function serializeSignature3({ r, s, to = "hex", v, yParity }) {
  const yParity_ = (() => {
...
async function sign({ hash, privateKey, to = "object" }) {
  ...
  return (() => {
    if (to === "bytes" || to === "hex")
      return serializeSignature3({ ...signature, to });
    return signature;
  })();
}
...
var presignMessagePrefix2 = `\x19Ethereum Signed Message:
`;
function toPrefixedMessage2(message_) {
  ...
  const prefix = stringToHex(`${presignMessagePrefix2}${size(message)}`);
  return concat([prefix, message]);
}
function hashMessage2(message, to_) {
  return keccak256(toPrefixedMessage2(message), to_);
}
...
function format(value2, decimals = 0) {
...
class InvalidLegacyVError2 extends BaseError {
  constructor({ v }) {
...
class InvalidSerializableTransactionError2 extends BaseError {
...
class InvalidStorageKeySizeError2 extends BaseError {
...
var sha2564 = sha2563;
function sha2565(value2, to_) {
  const to = to_ || "hex";
  const bytes = sha2564(isHex(value2, { strict: false }) ? toBytes(value2) : value2);
  if (to === "bytes")
    return bytes;
  return toHex(bytes);
}
...
class InvalidChainIdError2 extends BaseError {
  constructor({ chainId }) {
...
class FeeCapTooHighError2 extends BaseError {
...
class FeeCapTooLowError2 extends BaseError {
  constructor({ cause, maxFeePerGas } = {}) {
...
class NonceTooHighError2 extends BaseError {
...
class NonceTooLowError2 extends BaseError {
  constructor({ cause, nonce } = {}) {
    super([
...
class NonceMaxValueError2 extends BaseError {
  constructor({ cause, nonce } = {}) {
...
class InsufficientFundsError2 extends BaseError {
  constructor({ cause } = {}) {
    super([
...
class IntrinsicGasTooHighError2 extends BaseError {
...
class IntrinsicGasTooLowError2 extends BaseError {
  constructor({ cause, gas } = {}) {
...
class TransactionTypeNotSupportedError2 extends BaseError {
  constructor({ cause }) {
...
class TipAboveFeeCapError2 extends BaseError {
...
class AbiEncodingArrayLengthMismatchError2 extends BaseError {
...
class AbiEncodingBytesSizeMismatchError2 extends BaseError {
...
class AbiEncodingLengthMismatchError2 extends BaseError {
...
class InvalidAbiEncodingTypeError2 extends BaseError {
...
class InvalidArrayError2 extends BaseError {
  constructor(value2) {
...
function encodeNumber(value2, { signed, size: size3 = 256 }) {
  if (typeof size3 === "number") {
...
class InvalidPrimaryTypeError2 extends BaseError {
...
class InvalidStructTypeError2 extends BaseError {
  constructor({ type }) {
...
class InvalidTypedDataTypeError2 extends BaseError {
  constructor({ type }) {
...
function validateTypedData2(parameters) {
  ...
  const validateData = (struct, data) => {
    for (const param of struct) {
      ...
      if (baseType === "int" || baseType === "uint")
        throw new InvalidTypedDataTypeError2({ type });
  ...
  if (primaryType !== "EIP712Domain") {
    if (types4[primaryType])
      ...
      throw new InvalidPrimaryTypeError2({ primaryType, types: types4 });
  }
}
...
function encodeField({ types: types4, name, type, value: value2 }) {
  ...
  if (type.lastIndexOf("]") === type.length - 1) {
    ...
    return [
      { type: "bytes32" },
      keccak256(encodeAbiParameters2(typeValuePairs.map(([t]) => t), typeValuePairs.map(([, v]) => v)))
    ];
  }
  return [{ type }, value2];
}
...
var submitRawTx = (nodeRuntime, signedTx) => {
...
var rpcCall = (runtime2, httpClient, method, params, id) => {
  const { xdcRpcUrl } = runtime2.config;
  const bodyBytes = bytesToBase64(new TextEncoder().encode(JSON.stringify({ jsonrpc: "2.0", method, params, id })));
  const hexResult = httpClient.sendRequest(runtime2, (sendRequester) => {
    ...
    if (!ok(r))
      throw new Error(`${method} HTTP ${r.statusCode}`);
    return json(r).result;
  }, consensusIdenticalAggregation())().result();
  return hexResult;
};
var onHttpTrigger = async (runtime2, triggerEvent) => {
  const httpClient = new ClientCapability;
  const { xdcChainId } = runtime2.config;
  const { to, data, gasLimit = 300000 } = JSON.parse(new TextDecoder().decode(triggerEvent.input));
  const privateKey = runtime2.getSecret({ id: "cre_transaction_private_key" }).result().value;
  const account = privateKeyToAccount(privateKey);
  runtime2.log(`Sender: ${account.address}`);
  const nonceHex = rpcCall(runtime2, httpClient, "eth_getTransactionCount", [account.address, "pending"], 1);
  const nonce = parseInt(nonceHex, 16);
  const gasPriceHex = rpcCall(runtime2, httpClient, "eth_gasPrice", [], 2);
  const gasPrice = BigInt(gasPriceHex);
  runtime2.log(`Nonce: ${nonce}, gasPrice: ${gasPrice}`);
  ...
  runtime2.log(`TxHash: ${txHash}`);
  runtime2.runInNodeMode(submitRawTx, ConsensusAggregationByFields({ status: identical }))(signedTx).result();
  return JSON.stringify({ txHash, to, nonce, status: "submitted" });
};
var initWorkflow = (config) => {
  const http2 = new HTTPCapability;
  return [handler(http2.trigger({ authorizedKeys: [] }), onHttpTrigger)];
};
async function main() {
